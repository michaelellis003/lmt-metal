...
def print_results_table(
    ...
    results_by_config_lr: dict[
        str, dict[float, list[float]]
    ],
) -> dict[str, float]:
...
def main() -> None:
    ...
    total_runs = (
        len(configs) * len(LEARNING_RATES) * len(seeds)
    )
    ...
    results_by_config_lr: dict[
        ...
        name: {lr: [] for lr in LEARNING_RATES}
        for name in config_names
    }
    run_count = 0
    ...
    best_lr = print_results_table(
        config_names, results_by_config_lr,
    )

    print_best_lr_comparison(
        config_names, results_by_config_lr, best_lr,
    )
