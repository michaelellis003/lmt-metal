...
def compute_update_ratio(
    ...
    weight_sq = 0.0
    for (_, c), (_, p) in zip(
        cur_flat, prev_flat, strict=True,
    ):
        diff = c - p
        mx.eval(diff)
...
def train_one(
    ...
    description = (
        f'{run_name}, lr={learning_rate:.0e}, seed={seed}'
    )
    ...
    while not flop_counter.should_stop:
        for batch in batch_iterator(
            ...
            if step_count % eval_interval == 0:
                ...
                for cb in callbacks:
                    cb.on_eval_end(
                        step_count, eval_result,
                    )

    # Final evaluation
    final_result = evaluate(
        model, val_tokens, batch_size, seq_len,
    )
...
def print_comparison(
    ...
    for drop in DROPOUT_RATES:
        ...
        if gpt_key in results_by_key and llama_key in results_by_key:
            gpt_vals = [
                r['best_val_loss']
                for r in results_by_key[gpt_key]
            ]
            llama_vals = [
                r['best_val_loss']
                for r in results_by_key[llama_key]
            ]
            gpt_mean = sum(gpt_vals) / len(gpt_vals)
            llama_mean = (
                sum(llama_vals) / len(llama_vals)
            )
    ...
    print('\nDropout effect within each architecture:')
    print(
        f'  {"Arch":<8} {"Comparison":>14}'
        f' {"d":>8} {"Gap reduction":>14}'
    )
    print('  ' + '-' * 50)
    for arch in ['GPT', 'LLaMA']:
        ...
        base_vals = [
            r['best_val_loss']
            for r in results_by_key[base_key]
        ]
        base_gaps = [
            r['train_val_gap']
            for r in results_by_key[base_key]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            'HYP-001d: Dropout x Architecture ablation'
        ),
    )
    parser.add_argument(
        '--flop-budget',
    ...
    tokens = mx.array(
        tokenizer.encode(text), dtype=mx.int32,
    )
    split_idx = int(len(tokens) * 0.9)
    train_tokens = tokens[:split_idx]
    val_tokens = tokens[split_idx:]
    print(
        f'Tokenizer: char-level, '
    ...
    print(
        f'Total entries in log: {log_summary["total"]}'
    )
    print('=' * 78)
