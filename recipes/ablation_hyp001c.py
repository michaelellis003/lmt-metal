...
def build_ablation_configs(
    ...
    gated_block = replace(
        rope_block, ffn='gated', d_ff=d_ff_gated,
    )

    # 5. + GQA (4 KV heads for 8 query heads)
    gqa_block = replace(
        gated_block, attention='gqa', n_kv_heads=n_kv_heads,
    )
...
def train_one(
    ...
    """
    description = (
        f'{name}, lr={learning_rate:.0e}, seed={seed}'
    )
...
def print_comparison(
    ...
    print('\n' + '=' * 78)
    print(
        'Comparison vs GPT Baseline '
        '(best val loss, at best LR per config)'
    )
    print('=' * 78)
    print(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            'HYP-001c: FLOP-matched GPT->LLaMA ablation'
        ),
    )
    parser.add_argument(
        '--flop-budget',
    ...
    config_order_map = {
        name: i for i, (name, _) in enumerate(configs)
    }
    ...
    print(
        'Design fix: SwiGLU d_ff=341 '
        '(was 512 in HYP-001, per LIT-002)'
    )
    if args.mlflow:
    ...
    # --- Analysis ---
    print_results_table(
        config_names, val_by_config, train_by_config,
    )
